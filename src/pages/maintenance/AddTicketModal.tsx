...
export function AddTicketModal({ isOpen, propertyId, onClose, onSuccess }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null)
    try {
      await createTicket({ title, description, staffId, reportedId: currentUser.id, priority, roomId, propertyId })
      onSuccess()
      onClose()
    } catch {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-md mx-4" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-lg font-semibold text-white">{t('maintenance.addTicket')}</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-slate-400 mb-1.5">{t('maintenance.description')}</label>
            <textarea
              required
              value={description}
          ...
          <div>
            <label className="block text-xs font-medium text-slate-400 mb-1.5">{t('maintenance.priority')}</label>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={loading} className="flex-1 py-2.5 bg-cyan-500 hover:bg-cyan-400 disabled:opacity-50 text-white text-sm font-medium rounded-xl transition-colors">
              {loading ? t('common.creating') : t('maintenance.createTicket')}
            </button>
          </div>
        </form>
      </div>
