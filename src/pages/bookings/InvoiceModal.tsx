    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md mx-4 shadow-2xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-700">
          <h2 className="text-white font-semibold">{t('bookings.invoice')}</h2>
          ...
          {loading ? (
            ...
            <>
              <div className="flex items-center justify-between mb-6">
                ...
                <span className="text-xs text-slate-500">{t('bookings.issued')} {fmt(invoice.issuedAt)}</span>
              </div>

              <div className="mb-5">
                <p className="text-xs text-slate-500 mb-0.5">{t('bookings.guest')}</p>
                <p className="text-white font-medium">{invoice.guestName}</p>
              </div>

              <div className="grid grid-cols-2 gap-4 mb-5">
                <div>
                  <p className="text-xs text-slate-500 mb-0.5">{t('bookings.checkIn')}</p>
                  <p className="text-slate-300 text-sm">{fmt(invoice.checkInDate)}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-500 mb-0.5">{t('bookings.checkOut')}</p>
              ...
              <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4 mb-5 space-y-3">
                <div className="flex justify-between text-sm">
                  <span className="text-slate-400">{t('bookings.nights')}</span>
                  <span className="text-white">{invoice.nights}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-400">{t('bookings.pricePerNight')}</span>
                  <span className="text-white">{fmtCurrency(invoice.pricePerNight)}</span>
                </div>
                <div className="border-t border-slate-700 pt-3 flex justify-between">
                  <span className="text-white font-medium">{t('bookings.total')}</span>
